...
public class KJVMExporter extends KJVMPackageHandler {
	...
	public void exportClasses(File folder) {
		...
		try {
			...
			//native
			String nativeDispatcherImport = "/* Native method dispatcher - AUTOGENERATED by jvm exporter, DO NOT CHANGE!!!\n*/";
			String nativeMethodDefinitions= "";
			String nativeDispatchMethod = "\n\nvoid* nativeDispatch(const u2 classId,const u2 methodId,void* param){\n\tvoid* ret = NULL;\n";
			
			nativeDispatcherImport += "#include \"nativedispach.h\"\n";
			nativeDispatcherImport += "extern void* paramRead(void* in,void *out,char type,int paramIndex);\n";
				...
				if(kclass.isExport())
				{
					...
					for (int j = 0; j < methods.size(); j++)
					{
						...
						if(AccessFlags.isNative(method.getAccessFlags()))
						{
							...
							nativeDispatchMethod += "\n\tif(classId == " + cPK.getClassId(kclass.getFullClassName()) + " && methodId == " + j +"){";
							
							String methodCall = "\n\t\t";
							if(!returnType.getType().equals("void")){
								methodCall += getNativeType(returnType) +" r = ";
							}
							...
							for(int k = 0; k < params.size(); k++){
								...
								nativeDispatchMethod+= "\n\t\t" + paramDef +"; param = paramRead(param,&param"+k
													+",\'" + param.getTypeShort() + "\'," + (param.getDimensionCount() > 0 ? 1 : 0)+");";
			...
			nativeDispatchMethod += "\treturn ret;\n}\n";
