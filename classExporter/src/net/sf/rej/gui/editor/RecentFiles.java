...
public class RecentFiles {
	...
	public void load() throws IOException 
	{
		...
		try {
			...
			this.list.clear();
			recentOpenPath = this.properties.getProperty("recent.open");
			recentSavePath = this.properties.getProperty("recent.save");
	...
	public void add(String file) throws IOException {
		...
		if(recentOpenPath != null)
			this.properties.setProperty("recent.open", recentOpenPath);
		if(recentSavePath != null)
			this.properties.setProperty("recent.save", recentSavePath);
		for (int i = 0; i < 4; i++) {
			if (this.list.size() > i) {
	...
	public void setOpenPath(final String file){
		recentOpenPath = file;
		this.properties.setProperty("recent.open", recentOpenPath);
		try{
			serialize();
	...
	public void setSavePath(final String file){
		recentSavePath = file;
		this.properties.setProperty("recent.save", recentSavePath);
		try{
			serialize();
