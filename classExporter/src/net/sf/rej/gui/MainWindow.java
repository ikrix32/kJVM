...
public class MainWindow extends JFrame implements EventObserver {
    ...
    private Action openAction = new AbstractAction("Open..") {
        ...
        public void actionPerformed(ActionEvent e) 
        {
            MainWindow.this.fd.setFileSelectionMode(JFileChooser.FILES_AND_DIRECTORIES);
            ...
            String openPath = SystemFacade.getInstance().getRecentFiles().getOpenPath();
            if(openPath != null)
            	MainWindow.this.fd.setCurrentDirectory(new File(openPath));
            
            int i = MainWindow.this.fd.showDialog(instance, "Open");
            if (i == JFileChooser.APPROVE_OPTION) {
            	final File selectedFile = MainWindow.this.fd.getSelectedFile();
            	if(selectedFile.isDirectory())
            		SystemFacade.getInstance().getRecentFiles().setOpenPath(selectedFile.getAbsolutePath());
            	else
            		SystemFacade.getInstance().getRecentFiles().setOpenPath(selectedFile.getParentFile().getAbsolutePath());
    ...
    private Action saveAsAction = new AbstractAction("Save as..") {
        public void actionPerformed(ActionEvent e) {
            ...
            String savePath = SystemFacade.getInstance().getRecentFiles().getSavePath();
            if(savePath != null)
            	MainWindow.this.fd.setCurrentDirectory(new File(savePath));
            
            int i = MainWindow.this.fd.showDialog(instance, "Save");
            if (i == JFileChooser.APPROVE_OPTION) {
            	final File selectedFile = MainWindow.this.fd.getSelectedFile();
            	if(selectedFile.isDirectory())
            		SystemFacade.getInstance().getRecentFiles().setSavePath(selectedFile.getAbsolutePath());
            	else
            		SystemFacade.getInstance().getRecentFiles().setSavePath(selectedFile.getParentFile().getAbsolutePath());
    ...
    public void updateRecentFilesMenu() {
        ...
        List list = SystemFacade.getInstance().getRecentFiles().getList();
