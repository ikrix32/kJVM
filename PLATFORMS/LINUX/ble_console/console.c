...
static bool m_consoleConnected = false;
...
void console_print(uint8_t * p_data, int length){
	for(int i = 0; i < length;i++){
		buff[buffPointer] = p_data[i];
		const char crtChar = (char)buff[buffPointer];
		buffPointer++;
		if(crtChar == '\n' || buffPointer == BLE_NUS_MAX_DATA_LEN){
			uint32_t aa = ble_nus_send_string(&m_nus,(uint8_t*)&buff, buffPointer);
			if(aa != NRF_SUCCESS){
				int x =0;
				x = x/x;
			}
